...
use App\Models\MaximizeAsset;
use App\Repositories\Eloquent\GenericCrudRepository;
use App\Services\Media\UploadService;
use App\Services\Shared\CrudService;
use Illuminate\Http\Request;

class MaximizeAssetController extends Controller
{
    protected CrudService $service;

    protected string $routeIndex = 'maximize-assets.index';

    protected string $viewPrefix = 'admin.maximize-assets';

    /**
     * Legacy bug: MaximizeAsset uploaded to "our-clients" folder.
     * Fixed: uploads to "maximize-assets".
     */
    protected array $imageFields = [
        'image' => 'maximize-assets',
        'bannerImage' => 'maximize-assets',
    ];

    public function __construct(UploadService $uploadService)
    {
        $this->service = new CrudService(
            GenericCrudRepository::for(MaximizeAsset::class),
            $uploadService,
        );
    }
    ...
    public function index()
    {
        return view("{$this->viewPrefix}.index", [
            'data' => MaximizeAsset::orderBy('id', 'desc')->get(),
        ]);
    }
